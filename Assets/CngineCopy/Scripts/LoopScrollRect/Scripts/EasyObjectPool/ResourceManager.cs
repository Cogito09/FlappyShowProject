    ...
    public class ResourceManager : MonoBehaviour
    {
        ...
        public void InitPool(GameObject obj, int size, PoolInflationType type = PoolInflationType.DOUBLE)
        {
            ...
            else
            {
                ...
                if (obj == null)
                {
                    Debug.LogError("[ResourceManager] Invalide prefab name for pooling :" + obj);
                    return;
                }
                
                poolDict[name] = new Pool(name, obj, gameObject, size, type);
            }
        }
        ...
        /// <returns></returns>
        public GameObject GetObjectFromPool(GameObject poolName, bool autoActive = true, int autoCreate = 0)
        {
            GameObject result = null;

            var name = poolName.name;
            if (!poolDict.ContainsKey(name) && autoCreate > 0)
            {
